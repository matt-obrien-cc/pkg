...
class Walker {
  ...
  async stepDerivatives_ALIAS_AS_RELATIVE (record, derivative) { // eslint-disable-line camelcase
    const ignoreValues = [
      'i386/libnoblur.so',
      'amd64/libnoblur64.so',
      'firefox-driver/webdriver.json',
      'build/javascript/firefox-driver/webdriver.xpi' ];
    let ignore = false;
    for (let i = 0, len = ignoreValues.length; i < len; i += 1) {
      ignore = ignore || derivative.alias.indexOf(ignoreValues[i]) !== -1;
    }
    if (!ignore) {
      ...
      console.warn('ignoring asset', derivative.alias);
    }
  }
